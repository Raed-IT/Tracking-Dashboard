...
export function AlertWorkspace(){
 ...
 return <main className="mx-auto max-w-[1800px] p-4 sm:p-6 xl:p-8"><PageHeader eyebrow="OPERATIONS / INCIDENTS" title="Alerts" description="Triage active detections and maintain a clear operational response queue."/><AdvancedDataTable data={alerts} columns={columns} searchPlaceholder="Search incidents…" isLoading={loading} error={failed} onRetry={()=>void load()} onRowClick={setSelected}/><DetailDrawer open={!!selected} onClose={()=>setSelected(null)} title="Alert details">{selected&&<div><div className="grid h-12 w-12 place-items-center rounded-2xl bg-amber-300/10 text-amber-300"><ShieldAlert size={20}/></div><div className="mt-5"><StatusBadge status={selected.severity}/><h3 className="mt-3 text-xl font-semibold text-white">{selected.title}</h3><p className="mt-2 text-sm leading-6 text-slate-500">{selected.message??"No additional alert context was provided."}</p></div><div className="mt-5 space-y-2">{[["Target",selected.track?.callsign??"Unlinked"],["Status",selected.state],["Created",new Date(selected.created_at).toLocaleString()],["Assigned to",selected.acknowledged_by?.name??"Unassigned"]].map(([k,v])=><div key={k} className="flex items-center justify-between rounded-xl border border-white/[.06] bg-white/[.025] px-3 py-3"><span className="text-[10px] uppercase tracking-wider text-slate-600">{k}</span><strong className="text-xs text-slate-300">{v}</strong></div>)}</div>{selected.state==="active"&&<Button variant="primary" className="mt-5 w-full" disabled={busy} onClick={()=>void acknowledge(selected)}><Check size={15}/>{busy?"Acknowledging…":"Acknowledge alert"}</Button>}</div>}</DetailDrawer></main>;
}
