...
export function UserManager() {
  ...
  const [deleteTargets, setDeleteTargets] = useState<UserRecord[]>([]);
  ...
  const remove = async () => {
    ...
    setDeleting(true);
    try {
      await Promise.all(deleteTargets.map((user) => deleteUser(user.id)));
      showNotice("success", deleteTargets.length === 1
        ? `${deleteTargets[0].name} was removed successfully.`
        : `${deleteTargets.length} users were removed successfully.`);
      setDeleteTargets([]);
      await load();
    } catch {
  ...
  const columns: DataColumn<UserRecord>[] = [
    ...
    { id: "actions", header: "Actions", cell: (user) => <span className="flex gap-2"><Link href={`/admin/users/${user.id}/edit`} onClick={(event) => event.stopPropagation()}><Button size="sm">Edit</Button></Link><Button size="sm" variant="danger" disabled={user.id === current?.id} onClick={(event) => { event.stopPropagation(); setDeleteTargets([user]); }}><Trash2 size={14} /></Button></span> },
  ];

  return (
    <main className="mx-auto max-w-[1400px] p-4 sm:p-6 xl:p-8">
      <PageHeader eyebrow="Access control" title="Users" description="Manage accounts and their access roles." actions={<div className="flex flex-wrap gap-2"><Link href="/admin/users/create"><Button variant="primary"><Plus size={16} />Add user</Button></Link><Link href="/admin/roles"><Button variant="secondary"><Shield size={16} />Manage roles</Button></Link></div>} />
