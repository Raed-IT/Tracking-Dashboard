...
import { deleteOrganizationUser, fetchOrganizationUsersPage } from "@/services/api";
...
export function UserManager() {
  ...
  const load = useCallback(async () => {
    try {
      setError("");
      setLoading(true);
      const result = await fetchOrganizationUsersPage({ search: query, role: role || undefined, page, perPage: pageSize, sort, direction });
      setUsers(result.users);
      setTotal(result.total);
    } catch {
      const message = "Unable to load organization users. Check the selected filters and try again.";
      setError(message);
