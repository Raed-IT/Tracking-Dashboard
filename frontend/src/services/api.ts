...
export async function fetchTracks(bbox: string): Promise<Track[]> {
  const { data } = await api.get<{ data: Track[] }>("/tracks", {
    params: { bbox, per_page: 500 },
  });
  return data.data;
}
...
export type CreateSourceInput = {
  name: string;
  type: string;
  driver: "flightradar24" | "mock_aircraft";
  api_key?: string;
};
export async function createSource(
  input: CreateSourceInput,
): Promise<DataSource> {
  const { data } = await api.post<{ data: DataSource }>("/sources", input);
  return data.data;
}
export async function updateSource(
  ...
  const { data } = await api.patch<{ data: DataSource }>(
    `/sources/${id}`,
    input,
  );
  return data.data;
...
export async function login(
  ...
  const { data } = await api.post<{ token: string; user: AuthenticatedUser }>(
    "/auth/login",
    { email, password },
  );
  return data;
...
export async function acknowledgeAlert(id: string): Promise<OperatorAlert> {
  const { data } = await api.post<{ data: OperatorAlert }>(
