...
export type CreateSourceInput={name:string;type:string;driver:"flightradar24"|"mock_aircraft";api_key?:string};
export async function createSource(input:CreateSourceInput):Promise<DataSource>{const {data}=await api.post<{data:DataSource}>("/sources",input);return data.data}
export async function updateSource(id:string,input:{enabled:boolean}):Promise<DataSource>{const {data}=await api.patch<{data:DataSource}>(`/sources/${id}`,input);return data.data}
...
export type SystemStatus={status:"operational"|"degraded";checks:Record<string,string>;sources:Record<string,number>};
export async function fetchSystemStatus():Promise<SystemStatus>{const {data}=await api.get<SystemStatus>("/system/status");return data}
